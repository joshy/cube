...
Cube: {"": "Object;canvas,ctx,view,vertices,faces,colors,angle",
  ...
  get$draw: function() {
    return new H.BoundClosure$1(this, R.Cube.prototype.draw$1, null, "draw$1");
  },
  arrayToRgb$1: function(arr) {
    var t1 = arr.length;
    if (t1 === 3) {
      if (0 >= t1)
        throw H.ioore(arr, 0);
      t1 = C.JSString_methods.$add("rgb(", J.toString$0(arr[0])) + ",";
      if (1 >= arr.length)
        throw H.ioore(arr, 1);
      t1 = C.JSString_methods.$add(t1, J.toString$0(arr[1])) + ",";
      if (2 >= arr.length)
        throw H.ioore(arr, 2);
      return C.JSString_methods.$add(t1, J.toString$0(arr[2])) + ")";
    }
    return "rgb(0,0,0)";
  },
  loop$1: function(_, ctx) {
    var t, t1, i, r, t2, avg_z, f, t3, t4, t5;
    t = P.List_List(8, null);
    for (t1 = this.vertices, i = 0; i < 8; ++i) {
      r = t1[i].rotateX$1(this.angle).rotateY$1(this.angle).rotateZ$1(this.angle);
      t2 = this.view;
      t[i] = r.project$4(t2.width, t2.height, 200, 4);
    }
    avg_z = P.List_List(6, null);
    for (t1 = this.faces, i = 0; i < 6; ++i) {
      f = t1[i];
      t2 = f.length;
      if (0 >= t2)
        throw H.ioore(f, 0);
      ...
      if (t2 >>> 0 !== t2 || t2 >= 8)
        throw H.ioore(t, t2);
      avg_z[i] = H.fillLiteralMap(["index", i, "z", (t3 + t4 + t5 + t[t2].z) / 4], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    }
    H.IterableMixinWorkaround_sortList(avg_z, new R.Cube_loop_closure());
    for (t2 = this.colors, t3 = J.getInterceptor$x(ctx), i = 0; i < 6; ++i) {
      t4 = avg_z[i];
      t4 = t4.$index(t4, "index");
      if (t4 >>> 0 !== t4 || t4 >= 6)
        throw H.ioore(t1, t4);
      f = t1[t4];
      t4 = avg_z[i];
      ...
      if (t4 >>> 0 !== t4 || t4 >= 6)
        throw H.ioore(t2, t4);
      t3.set$fillStyle(ctx, this.arrayToRgb$1(t2[t4]));
      ctx.beginPath();
      if (0 >= f.length)
        throw H.ioore(f, 0);
      t4 = f[0];
...
Cube$: function(canvas) {
  var t1 = new R.Cube(canvas, null, null, [new R.Point3D(-1, 1, -1), new R.Point3D(1, 1, -1), new R.Point3D(1, -1, -1), new R.Point3D(-1, -1, -1), new R.Point3D(-1, 1, 1), new R.Point3D(1, 1, 1), new R.Point3D(1, -1, 1), new R.Point3D(-1, -1, 1)], [[0, 1, 2, 3], [1, 5, 6, 2], [5, 4, 7, 6], [4, 0, 3, 7], [0, 4, 5, 1], [3, 2, 6, 7]], [[241, 238, 246], [208, 209, 230], [166, 189, 219], [116, 169, 207], [43, 140, 190], [4, 90, 141]], 0);
  t1.Cube$1(canvas);
  return t1;
}}
...
Cube_loop_closure: {"": "Closure;",
  call$2: function(a, b) {
    return J.$sub$n(J.$index$asx(b, "z"), J.$index$asx(a, "z"));
  },
  $is_args2: true
},

Point3D: {"": "Object;x,y,z",
  rotateX$1: function(angle) {
    ...
    t1 = this.y;
    t2 = this.z;
    return new R.Point3D(this.x, t1 * cosa - t2 * sina, t1 * sina + t2 * cosa);
  },
  rotateY$1: function(angle) {
    var rad, cosa, sina, t1, t2;
    rad = angle * 3.141592653589793 / 180;
    cosa = Math.cos(rad);
    sina = Math.sin(rad);
    t1 = this.z;
    t2 = this.x;
    return new R.Point3D(t1 * sina + t2 * cosa, this.y, t1 * cosa - t2 * sina);
  },
  rotateZ$1: function(angle) {
    ...
    t1 = this.x;
    t2 = this.y;
    return new R.Point3D(t1 * cosa - t2 * sina, t1 * sina + t2 * cosa, this.z);
  },
  project$4: function(viewHeight, viewWidth, fov, viewDistance) {
    var t1, factor;
    t1 = this.z;
    factor = fov / (viewDistance + t1);
    if (typeof viewWidth !== "number")
      throw viewWidth.$div();
    if (typeof viewHeight !== "number")
      throw viewHeight.$div();
    return new R.Point3D(this.x * factor + viewWidth / 2, this.y * factor + viewHeight / 2, t1);
  }
}},
1],
...
function dart_precompiled($collectedClasses) {
  ...
  function Cube(canvas, ctx, view, vertices, faces, colors, angle) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.view = view;
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Cube.prototype = $desc;
  function Cube_loop_closure() {
  }
  Cube_loop_closure.builtin$cls = "Cube_loop_closure";
  if (!"name" in Cube_loop_closure)
    Cube_loop_closure.name = "Cube_loop_closure";
  $desc = $collectedClasses.Cube_loop_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Cube_loop_closure.prototype = $desc;
  function Point3D(x, y, z) {
    this.x = x;
    this.y = y;
    this.z = z;
  }
  Point3D.builtin$cls = "Point3D";
  if (!"name" in Point3D)
    Point3D.name = "Point3D";
  $desc = $collectedClasses.Point3D;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Point3D.prototype = $desc;
